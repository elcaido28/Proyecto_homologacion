...
En atención a la solicitud de revisión de malla, presentada por la/el Sr/Srta. '.$nombres.', me permito manifestar a usted y por su digno intermedio a los Miembros del H. Consejo Directivo, que luego de efectuada la revisión y análisis de la documentación presentada, proceden las siguientes consideraciones:
  '),0,'J',0);
  ...
  $y=$pdf->GetY();
  $pdf->SetXY(-190,$y);
$pdf->MultiCell(165,5,'Se le convalida la/s materia/s de: '.$xqno.'',0,'J',0);
...
$consul11=mysqli_query($con,"SELECT * from  homologaciones where numero_ofi='$nof' ORDER BY fechaH DESC");
 while($row11= mysqli_fetch_array($consul11)){
 $fech=$row11['fechaH'];
}
$y=$pdf->GetY();
$pdf->SetXY(-190,$y);
 $pdf->SetFont('arial','B',12);
$pdf->MultiCell(175,5,utf8_decode('
En base a lo expuesto, a el/la  Sr/Srta. '.$nombres.', se hace un alcance al Oficio Nº: '.$nof.', con fecha '.$fech.', particular que pongo en consideración del H. Consejo Directivo para su análisis y resolución.
